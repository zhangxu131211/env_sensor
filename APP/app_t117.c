...
APP_T117_ErrCode APP_T117_Init(void) {
    ...
    if (!BSP_T117_Init(&hi2c2, s_t117Buffer)) {
        printf("[T117] Init Err: BSP init failed\r\n");
...
APP_T117_ErrCode APP_T117_ReadTemperature(float *pTemp) {
    // 1. 参数校验
    if (pTemp == NULL) {
        printf("[T117] Read Err: Param is NULL\r\n");
...
void APP_T117_Test(void) {
    ...
    err = APP_T117_Init();
    if (err != APP_T117_OK) {
        printf("[T117] Test Err: Init failed (err=%d)\r\n", err);
        while (1) {
    ...
    // 2. 循环读取温度并打印
    printf("[T117] Test Start: Read temp every %dms\r\n", APP_T117_READ_DELAY);
    while (1) {
        // 读取温度
        ...
        if (err == APP_T117_OK) {
            ...
            printf("[T117] Read Temp Failed (err=%d)\r\n", err);
        }
        // 延时
